...
if ( ! function_exists( 'fcbkbttn_admin_menu' ) ) {
	...
	function fcbkbttn_admin_menu() {
		...
		if ( ! is_plugin_active( 'facebook-button-pro/facebook-button-pro.php' ) &&
			...
			add_submenu_page(
				'facebook-button-plugin.php',
				esc_html__( 'Statistics', 'facebook-button-plugin' ),
				esc_html__( 'Statistics', 'facebook-button-plugin' ),
				'manage_options',
	...
	function fcbkbttn_admin_init() {
		...
		if ( empty( $bws_plugin_info ) ) {
			$bws_plugin_info = array(
				'id' => '78',
				'version' => $fcbkbttn_plugin_info['Version'],
			);
		}
		...
		<div class="wrap">
			...
			<div class="bws_pro_version_bloc">
				...
				<div class="bws_pro_version_tooltip">
					<a class="bws_button" href="https://bestwebsoft.com/products/wordpress/plugins/facebook-like-button/?k=427287ceae749cbd015b4bba6041c4b8&amp;pn=78&amp;v=<?php echo esc_attr( $fcbkbttn_plugin_info['Version'] ); ?>&amp;wp_v=<?php echo esc_attr( $wp_version ); ?>" target="_blank" title="Like & Share Pro"><?php esc_html_e( 'Upgrade to Pro', 'facebook-button-plugin' ); ?></a>
					<div class="clear"></div>
				</div>
			</div>
			<?php else : ?>
			<p>
				<?php
				esc_html_e( 'This tab contains Pro options only.', 'facebook-button-plugin' );
				echo ' ' . sprintf(
					esc_html__( '%1$sChange the settings%2$s to view the Pro options.', 'facebook-button-plugin' ),
...
if ( ! function_exists( 'fcbkbttn_button' ) ) {
	...
	function fcbkbttn_button() {
		...
		$if_large = '';
		if ( 'large' === $fcbkbttn_options['size'] ) {
...
if ( ! function_exists( 'fcbkbttn_function_display_arhive' ) ) {
	/**
	 * Function for display button on the Archive page
	 */
...
if ( ! function_exists( 'fcbkbttn_meta' ) ) {
	...
	function fcbkbttn_meta() {
		...
		if ( ( ! empty( $fcbkbttn_options['like'] ) || ! empty( $fcbkbttn_options['share'] ) ) && 1 === absint( $fcbkbttn_options['display_for_open_graph'] ) && ! post_password_required( get_the_ID() ) ) {
			if ( is_singular() ) {
				$image = '';
				...
				$url            = apply_filters( 'fcbkbttn_meta_url', get_permalink() );
				$description    = apply_filters( 'fcbkbttn_meta_description', $description );
				$title          = apply_filters( 'fcbkbttn_meta_title', get_the_title() );
				$site_name      = apply_filters( 'fcbkbttn_meta_site_name', get_bloginfo() );
				$meta_image     = apply_filters( 'fcbkbttn_meta_image', $image );
				print "\n" . '<!-- fcbkbttn meta start -->';
			...
			if ( is_archive() ) {
				...
				$url            = apply_filters( 'fcbkbttn_meta_url', $permalink_page );
				$description    = apply_filters( 'fcbkbttn_meta_description', $description );
				$title          = apply_filters( 'fcbkbttn_meta_title', $name );
				$site_name      = apply_filters( 'fcbkbttn_meta_site_name', get_bloginfo() );
...
if ( ! function_exists( 'fcbkbttn_get_locale' ) ) {
	/**
	 * Function for get locale for button
	 */
...
if ( ! function_exists( 'fcbkbttn_footer_script' ) ) {
	...
	function fcbkbttn_footer_script() {
		...
		if ( isset( $fcbkbttn_shortcode_add_script ) ||
			...
			/* echo '<script async crossorigin="anonymous" src="' . $fcbkbttn_sdk_script . '"></script>'; */
			wp_enqueue_script( 'fcbkbttn_sdk_script', $fcbkbttn_sdk_script, array(), 'v16.0', true );
			wp_script_add_data( 'fcbkbttn_sdk_script', array( 'crossorigin', 'strategy' ), array( 'anonymous', 'async' ) );
		}
	}
}
...
if ( ! function_exists( 'fcbkbttn_enqueue_scripts' ) ) {
	...
	function fcbkbttn_enqueue_scripts() {
		...
		wp_enqueue_style( 'fcbkbttn_icon', plugins_url( 'css/icon.css', __FILE__ ), false, $fcbkbttn_plugin_info['Version'] );
		if ( isset( $_GET['page'] ) && ( 'facebook-button-plugin.php' === $_GET['page'] || 'social-buttons.php' === $_GET['page'] ) ) {
			bws_enqueue_settings_scripts();
			bws_plugins_include_codemirror();
			wp_enqueue_script( 'fcbkbttn_script', plugins_url( 'js/admin-script.js', __FILE__ ), array( 'jquery' ), $fcbkbttn_plugin_info['Version'], true );
			wp_enqueue_media();
			wp_localize_script(
				'fcbkbttn_script',
				'fcbkbttn_var',
			...
			wp_enqueue_style( 'fcbkbttn_stylesheet', plugins_url( 'css/style.css', __FILE__ ), false, $fcbkbttn_plugin_info['Version'] );
		} elseif ( ! is_admin() ) {
			wp_enqueue_style( 'fcbkbttn_stylesheet', plugins_url( 'css/style.css', __FILE__ ), false, $fcbkbttn_plugin_info['Version'] );
			wp_enqueue_script( 'fcbkbttn_script', plugins_url( 'js/script.js', __FILE__ ), array( 'jquery' ), $fcbkbttn_plugin_info['Version'], true );
		}
	}
}
